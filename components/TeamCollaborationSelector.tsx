      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        {options.map((option: any) => (
          <button key={option.value} type="button" onClick={() => onChange(option.value)}
            className={`p-3 rounded-xl border-2 text-left transition-all ${value === option.value ? 'border-green-600 bg-green-50 text-green-900' : 'border-gray-100 bg-white text-gray-600 hover:border-gray-300'}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="border border-gray-200 rounded-2xl overflow-hidden">
        <button type="button" onClick={() => toggle('structure')} className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100">
          <div className="flex items-center gap-3"><Building2 className="w-5 h-5 text-indigo-600" /><span className="font-bold">Team Structure</span></div>
      ...
      <div className="border border-gray-200 rounded-2xl overflow-hidden">
        <button type="button" onClick={() => toggle('collab')} className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100">
          <div className="flex items-center gap-3"><MessageSquare className="w-5 h-5 text-teal-600" /><span className="font-bold">Collaboration</span></div>
