...
const CandidateOnboarding: React.FC<Props> = ({ profile, onUpdate, onComplete, onSaveExit, isSaving }) => {
  ...
  const renderContent = () => {
    switch (step.id) {
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-4 border border-gray-200 rounded-2xl font-bold bg-white"
              >
              ...
              <p className="text-xs text-gray-400 mt-1">Important for remote work matching</p>
            </div>

            {/* Headline - Optional */}
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Headline</label>
              <input value={profile.headline} onChange={e => onUpdate({ headline: e.target.value })} className="w-full p-4 border border-gray-200 rounded-2xl font-bold" placeholder="e.g., Senior Full-Stack Engineer" />
            </div>

            {/* Bio - Optional */}
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Bio</label>
              <textarea value={profile.bio} onChange={e => onUpdate({ bio: e.target.value })} rows={4} className="w-full p-4 border border-gray-200 rounded-2xl" placeholder="Tell us about yourself..." />
            </div>
          </div>
        </div>
      );
      case 'experience': return (
        <div className="space-y-8 max-w-4xl mx-auto">
          <div className="text-center">
            <h2 className="text-4xl font-black text-gray-900 mb-2">Impact Level</h2>
            <p className="text-gray-500">What scope of impact do you have, and where do you want to go?</p>
          </div>
      ...
      case 'culture': return (
        <div className="space-y-8 max-w-4xl mx-auto">
          <div className="text-center">
            <h2 className="text-4xl font-black text-gray-900 mb-2">Culture & Dynamics</h2>
            <p className="text-gray-500">What values and work environment help you thrive? (Optional but improves matching)</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Select up to 5 values that resonate with you.</p>
            <GroupedMultiSelect label="" options={CULTURAL_VALUES} selected={profile.values || []} onChange={v => onUpdate({ values: v })} maxSelections={5} />
          </div>

          {/* Work Style */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="text-lg font-bold mb-4 flex items-center"><Clock className="w-5 h-5 mr-2 text-blue-500"/> Work Style</h3>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-3">Team Distribution</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-3">Timezone Overlap Requirement</label>
          ...
          {/* Character Traits */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="text-lg font-bold mb-4 flex items-center"><Smile className="w-5 h-5 mr-2 text-purple-500"/> Character Traits</h3>
            <p className="text-sm text-gray-500 mb-4">How would colleagues describe you? Select up to 8.</p>
      ...
      case 'preferences': return (
        <div className="space-y-8 max-w-2xl mx-auto">
          <div className="text-center"><h2 className="text-4xl font-black text-gray-900 mb-2">Logistics</h2><p className="text-gray-500">How and where do you want to work?</p></div>
          ...
          {profile.location && (
            <div className="bg-gray-50 rounded-2xl p-4 flex items-center gap-3">
              ...
              <button onClick={() => setIdx(0)} className="ml-auto text-xs text-blue-600 font-bold hover:text-blue-700">Edit</button>
            </div>
          )}

          {/* Work Mode Selection */}
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase mb-3">Preferred Work Mode</label>
            <div className="flex flex-wrap gap-2">
              {Object.values(WorkMode).map(m => (
                <button
                  ...
                  className={`px-5 py-3 rounded-xl border-2 font-bold transition-all ${profile.preferredWorkMode?.includes(m) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-200 hover:border-gray-400'}`}
                >
                  {m}
                </button>
              ))}
      ...
      case 'review': return (
        <div className="space-y-8 max-w-4xl mx-auto">
          <div className="text-center"><h2 className="text-4xl font-black text-gray-900 mb-2">Review & Complete</h2><p className="text-gray-500">Make sure everything looks good before going live.</p></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="sticky bottom-0 bg-white border-t border-gray-100 p-4 md:p-6 z-50">
