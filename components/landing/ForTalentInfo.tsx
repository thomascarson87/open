...
/* Added missing Lock and Users imports from lucide-react */
import { 
    Zap, User, ShieldCheck, Search, MessageSquare, Heart, Clock, CheckCircle, 
    X, Layout, Target, Award, ArrowRight, ArrowLeft, Unlock, Smile, Globe, 
    Send, Gift, MousePointer2, Star, Lock, Users
} from 'lucide-react';
...
const ForTalentInfo: React.FC<Props> = ({ onGetStarted, onBack }) => {
    ...
    const frustrations = [
        { emoji: '📋', title: 'Application Black Holes', desc: 'You spend 45 minutes on a custom cover letter. Never hear back. Just... silence.' },
        { emoji: '🤖', title: 'Keyword Bingo', desc: '"Must have 10 years experience in a 5-year-old framework." Your actual skills? Irrelevant if the bot hates your resume.' },
        { emoji: '📧', title: 'Recruiter Spam', desc: '"I have an AMAZING opportunity!" It\'s a 3-month contract in a city you don\'t live in for half your salary.' },
        { emoji: '🎭', title: 'Culture Roulette', desc: 'The job post said "fast-paced." You wanted "sustainable pace." They actually meant "burnout central."' },
        { emoji: '💰', title: 'Salary Guessing Games', desc: 'Five interview rounds before they mention the salary. It\'s 30% below your minimum. Great.' },
        { emoji: '👻', title: 'The Ghosting', desc: 'Three interviews, a technical assessment, "we\'ll be in touch!" That was six weeks ago.' }
    ];

    const benefits = [
        { icon: '🎯', title: 'Matched, Not Searched', desc: 'See every job that fits the second you sign up. No searching required.', detail: '8-dimensional matching across skills, values, culture, and more.' },
        { icon: '🔒', title: 'You Set the Rules', desc: 'Non-negotiables are non-negotiable. If remote is a must, you only see remote.', detail: 'Salary floor, work mode, company size — you decide.' },
        { icon: '📈', title: 'Skills That Matter', desc: 'We measure what you can actually DO, not just how long you\'ve done it.', detail: '5 proficiency levels that show your growth trajectory.' },
        { icon: '✅', title: 'Verified Trust', desc: 'Actual colleagues vouch for your skills. Not social "endorsements."', detail: 'Companies trust verified data, getting you 3x more unlocks.' },
        { icon: '💬', title: 'Direct Conversations', desc: 'When a company matches you, you chat directly. No recruiters playing telephone.', detail: 'Integrated scheduling and tracking.' },
        { icon: '❤️', title: 'Find Your Vibe', desc: 'Match on work style and team cadence. Find the team you actually fit with.', detail: 'Work intensity, autonomy, and collaboration style alignment.' }
    ];
    ...
    const tabs = [
        { id: 'preferences', title: 'Your Rules', icon: <Lock className="w-4 h-4" /> },
        { id: 'skills', title: 'Actual Proficiency', icon: <Zap className="w-4 h-4" /> },
        { id: 'verification', title: 'Real Proof', icon: <ShieldCheck className="w-4 h-4" /> },
        { id: 'matching', title: 'Jobs That Fit', icon: <Target className="w-4 h-4" /> }
    ];
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 py-10 space-y-24">
            ...
            <section className="text-center animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-purple-50 text-purple-700 text-xs font-black uppercase tracking-widest border border-purple-100 mb-8">
                    <span className="mr-2">🚀</span> Free Forever • No Recruiter Spam
                </div>
            ...
            <section className="space-y-12 bg-white rounded-[3rem] p-12 md:p-16 border border-gray-100 shadow-sm">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {frustrations.map((f, i) => (
                        <div key={i} className="bg-gray-50 rounded-3xl p-8 border border-gray-100/50 hover:bg-white hover:shadow-md transition-all group">
                            <div className="text-4xl mb-4 grayscale group-hover:grayscale-0 transition-all transform group-hover:scale-110">{f.emoji}</div>
            ...
            <section id="features" className="space-y-16">
                <div className="text-center max-w-2xl mx-auto">
                    <div className="inline-flex items-center px-3 py-1 rounded-full bg-purple-100 text-purple-700 text-[10px] font-black uppercase tracking-widest mb-4">
                        ✨ The Open Way
                    </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {benefits.map((b, i) => (
                        <div key={i} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:border-purple-200 hover:shadow-xl transition-all relative group overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <div className="w-12 h-12 bg-purple-100 text-purple-600 rounded-xl flex items-center justify-center text-2xl mb-6 shadow-inner group-hover:scale-110 transition-transform">
                                    {b.icon}
                                </div>
            ...
            <section className="bg-white rounded-[3rem] border border-gray-200 overflow-hidden shadow-sm">
                <div className="flex border-b border-gray-100 overflow-x-auto no-scrollbar bg-gray-50/50">
                    {tabs.map(t => (
                        ...
                        >
                            <span className="flex items-center gap-2">{t.icon} {t.title}</span>
                        </button>
                    ))}
                </div>
            ...
            <section id="demo" className="space-y-12">
                ...
                <div className="bg-white rounded-[3rem] border border-gray-200 shadow-2xl overflow-hidden animate-in zoom-in duration-700">
                            ...
                            {activeDemo === 'verify' && (
                                <div className="space-y-6 animate-in fade-in duration-500">
                                    ...
                                    <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3 text-xs font-bold text-purple-700 bg-white p-3 rounded-xl border border-purple-100">
                                                <Star className="w-4 h-4 text-purple-600 fill-purple-600" /> Proficiency verified: Mastered React
                                            </div>
                                            <div className="flex items-center gap-3 text-xs font-bold text-purple-700 bg-white p-3 rounded-xl border border-purple-100">
                                                <Smile className="w-4 h-4 text-purple-600" /> Collaboration score: 9.5 / 10
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-purple-600 px-10 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white"><Clock className="w-5 h-5"/></div>
                            <span className="text-purple-100 text-sm font-bold uppercase tracking-widest">Takes 10 minutes to setup</span>
                        </div>
                        <button 
                            onClick={onGetStarted}
