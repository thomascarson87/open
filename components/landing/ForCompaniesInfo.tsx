...
/* Added missing Smile, Clock, and ArrowLeft imports from lucide-react */
import { 
    Zap, Briefcase, MousePointer2, ShieldCheck, BarChart3, 
    Search, Video, MessageSquare, ArrowRight, CheckCircle, 
    X, Layout, Target, Heart, DollarSign, Globe, Code, PlusCircle,
    Smile, Clock, ArrowLeft
} from 'lucide-react';
...
const ForCompaniesInfo: React.FC<Props> = ({ onGetStarted, onBack }) => {
    ...
    const features = [
        { id: 'widget', title: 'White-Label Widget', icon: <Code className="w-4 h-4" /> },
        { id: 'matching', title: 'Precision Matching', icon: <Target className="w-4 h-4" /> },
        { id: 'approvals', title: 'Approvals', icon: <ShieldCheck className="w-4 h-4" /> },
        { id: 'savings', title: 'Cost Savings', icon: <DollarSign className="w-4 h-4" /> }
    ];

    const oldWayItems = [
        { icon: '💸', text: '$15K-30K per hire to agencies' },
        { icon: '📋', text: 'Mass applications from job boards' },
        { icon: '⏳', text: 'Weeks screening misaligned candidates' },
        { icon: '🎲', text: 'Culture fit? Hope for the best' },
        { icon: '📊', text: 'No data on what makes hires succeed' }
    ];

    const openWayItems = [
        { icon: '💰', text: '$50 per candidate unlock' },
        { icon: '🎯', text: 'Pre-matched on 8 dimensions' },
        { icon: '⚡', text: 'Match scores tell you who to call' },
        { icon: '🧬', text: 'Skills, values, culture - all aligned' },
        { icon: '📈', text: 'Data-driven hiring that enriches over time' }
    ];
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 py-10 space-y-24">
            ...
            <section className="text-center animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-black uppercase tracking-widest border border-blue-100 mb-8">
                    <span className="mr-2">✨</span> Free Widget • No Credit Card
                </div>
                            ...
                            <ul className="space-y-6">
                                {oldWayItems.map((item, i) => (
                                    <li key={i} className="flex items-start gap-4 text-gray-400">
                                        <span className="text-xl bg-gray-50 w-10 h-10 rounded-xl flex items-center justify-center grayscale">{item.icon}</span>
                                        <span className="font-bold line-through mt-2">{item.text}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                            ...
                            <ul className="space-y-6">
                                {openWayItems.map((item, i) => (
                                    <li key={i} className="flex items-start gap-4 group/item">
                                        <span className="text-xl bg-white/10 w-10 h-10 rounded-xl flex items-center justify-center transition-transform group-hover/item:scale-110">{item.icon}</span>
                                        <span className="font-bold text-lg leading-tight mt-1.5">{item.text}</span>
                                    </li>
                                ))}
                            </ul>
                        ...
                        <div className="grid md:grid-cols-2 gap-16 items-center animate-in fade-in duration-500">
                            <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {[
                                        { l: '8 Match Dimensions', i: <Layout /> },
                                        { l: 'Behavioral Skills', i: <Zap /> },
                                        { l: 'Culture Alignment', i: <Heart /> },
                                        /* Smile is now imported correctly from lucide-react */
                                        { l: 'Personality Fit', i: <Smile /> }
                                    ].map(box => (
                                        <div key={box.l} className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                            <div className="text-blue-600 mb-2">{box.i}</div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex items-center gap-4 p-4 bg-yellow-50 rounded-2xl border border-yellow-100">
                                        /* Clock is now imported correctly from lucide-react */
                ...
                <div className="bg-white rounded-[3rem] border border-gray-200 shadow-2xl overflow-hidden animate-in zoom-in duration-700">
                    ...
                    <div className="bg-gray-900 px-10 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white"><Zap className="w-5 h-5"/></div>
                            <span className="text-gray-400 text-sm font-bold uppercase tracking-widest">Setup completed in 5 minutes</span>
                        </div>
                        <button 
                            onClick={onGetStarted}
            ...
            <div className="text-center pb-12">
                ...
                >
                    /* ArrowLeft is now imported correctly from lucide-react */
