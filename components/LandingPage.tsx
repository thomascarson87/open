...
import { User, Briefcase, ArrowRight, CheckCircle, Zap, MapPin, DollarSign, Clock, Layout, Globe, Star } from 'lucide-react';
...
const LandingPage: React.FC<Props> = ({ onSelectRole, onNavigate }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-900">
            {/* Header */}
            <header className="px-6 py-6 max-w-7xl mx-auto w-full flex items-center justify-between sticky top-0 z-50 bg-gray-50/80 backdrop-blur-sm">
                ...
                <div className="flex items-center space-x-4">
                    <button onClick={() => onSelectRole('candidate')} className="text-sm font-medium text-gray-500 hover:text-gray-900 hidden sm:block">For Talent</button>
                    <button onClick={() => onSelectRole('recruiter')} className="text-sm font-medium text-gray-500 hover:text-gray-900 hidden sm:block">For Companies</button>
                    <button onClick={() => onSelectRole('candidate')} className="bg-black text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-gray-800 transition-colors">
                        Get Started
                    </button>
                </div>
            </header>

            <main className="flex-grow flex flex-col items-center px-4 pb-20 pt-10">
                
                {/* Hero */}
                <div className="text-center max-w-4xl mx-auto mb-20 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    <div className="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-bold uppercase tracking-wider border border-blue-100">
                        <Zap className="w-3 h-3 mr-1" /> The future of technical hiring
                    </div>
